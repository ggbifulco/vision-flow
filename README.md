...
VisionFlow is an advanced multimodal video monitoring engine. It uses **YOLO26** for real-time object detection as a smart trigger and **Groq cloud API (Llama 4 Scout 17B)** for deep semantic scene understanding — no local VLM model required.
...
```
Camera → YOLO (local, real-time) → Smart Trigger → Groq VLM (cloud, ~1-2s) → Alerts
```

- **YOLO26** runs locally and handles real-time object detection at 30+ FPS.
- When a trigger condition is met (people, vehicles, pets detected), a frame is sent to **Groq** cloud.
- **Llama 4 Scout 17B** on Groq performs semantic scene analysis and returns a detailed description in ~1-2 seconds.
...
-   **Dual-Engine Inference**: Real-time object detection (YOLO26, local) combined with deep semantic reasoning (Groq Llama 4 Scout, cloud).
...
-   **Smart Triggering**: The Groq VLM analysis is triggered only when YOLO detects specific classes (people, vehicles, pets), keeping cloud calls efficient.
-   **Interactive Web Dashboard**: Modern Dark UI with live MJPEG streaming, real-time logs, and interactive Q&A.
...
Since the VLM runs on **Groq cloud**, VisionFlow is fast even on CPU-only machines. No GPU is needed.

- **YOLO** runs locally and is lightweight enough for any modern CPU.
- **VLM (Llama 4 Scout)** runs on Groq cloud — all you need is an API key and an internet connection.
- Expect 30+ FPS for YOLO detection and ~1-2s for semantic analysis via Groq.
